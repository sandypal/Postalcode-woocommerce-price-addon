...
class ProductPricing {
    ...
    public function box(){
        add_meta_box('bc-wcpa-pricing', 'Per-Pincode Pricing (Retail/Trader/Bulker)', [$this,'render'], 'product', 'normal', 'high');
    }

    public function assets($hook){
        if ( $hook !== 'post.php' && $hook !== 'post-new.php' ) return;
        wp_enqueue_script('bc-wcpa-admin', plugins_url('src/Admin/assets/admin.js', dirname(__DIR__,2) . '/buycement-woocommerce-pricing-addon.php'), ['jquery'], rand(1,9999), true);
        wp_localize_script('bc-wcpa-admin', 'BCWCPA', [
            'nonce' => wp_create_nonce('bc_wcpa_save'),
            'ajax'  => admin_url('admin-ajax.php')
        ]);
        wp_enqueue_style('bc-wcpa-admin', plugins_url('src/Admin/assets/admin.css', dirname(__DIR__,2) . '/buycement-woocommerce-pricing-addon.php'), [], rand(1,9999));
    }
    ...
    public function render( $post ){
        ...
        $prices = $wpdb->get_results( $wpdb->prepare("SELECT role,pincode_id,price FROM $prices_t WHERE product_id=%d", $post->ID ) );
       
        echo '<p>Set specific prices for each pincode and user role.</p>';
        echo '<table class="widefat fixed bc-wcpa-table"><thead><tr><th>Pincode</th><th>Guest</th><th>Retail</th><th>Trader</th><th>Bulker</th><th>Active</th></tr></thead><tbody>';
        
        foreach($pincodes as $p){
            $g = $this->get_price_for($prices, $p->id, 'guest');
            $r = $this->get_price_for($prices, $p->id, 'retail');
            $t = $this->get_price_for($prices, $p->id, 'trader');
            $b = $this->get_price_for($prices, $p->id, 'bulker');
            $active = $this->get_active_for($prices, $p->id);
            echo '<tr>';
            echo '<td>'.esc_html($p->pincode).'</td>';
             echo '<td><input type="number" step="0.01" name="price['.intval($p->id).'][guest]" value="'.esc_attr($g).'" /></td>';
            ...
            echo '<td><input type="checkbox" name="price['.intval($p->id).'][active]" value="1" '.checked($active, 1, false).' /></td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
        ...
        private function get_active_for($prices, $pincode_id) {
            foreach ($prices as $price) {
                if ($price->pincode_id == $pincode_id && isset($price->active)) {
                    return (int) $price->active;
                }
            }
    ...
    public function ajax_save(){
        ...
        foreach($prices as $pinId => $roles){
            ...
            foreach(['guest','retail','trader','bulker'] as $role){
             ...
             $active = isset($roles['active']) ? (int)$roles['active'] : 1;
    $wpdb->update($prices_t, ['active'=>$active], ['product_id'=>$product_id,'pincode_id'=>$pinId], ['%d'], ['%d','%d']);
        }
